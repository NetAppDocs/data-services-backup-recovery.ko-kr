...
SnapCenter는 리소스를 호스트에 매핑하기 위한 스토리지 클러스터 또는 스토리지 VM을 지원하지 않지만, NetApp Backup and Recovery는 지원합니다. 온프레미스 ONTAP 클러스터 또는 스토리지 VM은 NetApp Backup and Recovery 미리 보기 버전에서 호스트에 매핑되지 않습니다. 또한 NetApp Console은 스토리지 VM을 지원하지 않습니다.
