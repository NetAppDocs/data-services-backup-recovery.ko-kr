...
.개체 저장소 고려 사항에서 복원
객체 스토리지에서 백업 파일을 선택할 때 해당 백업에 대해 랜섬웨어 보호 기능이 활성화되어 있는 경우(백업 정책에서 Datalock 및 Ransomware 보호를 활성화한 경우), 데이터를 복원하기 전에 백업 파일에 대한 추가 무결성 검사를 실행하라는 메시지가 표시됩니다. 이 검사를 수행하는 것이 좋습니다.
