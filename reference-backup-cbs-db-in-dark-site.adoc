...
클라우드 계정을 사용하지 않고 온프레미스에서 _표준 모드_로 Backup and Recovery를 사용할 때 Console 에이전트를 교체하고 에이전트에 저장된 구성 데이터를 복원하려면 다음 단계를 따르십시오(예: NetApp StorageGrid를 백업 대상으로 사용하는 경우).
