...
summary: '개인 모드(인터넷 연결 없음)에서 NetApp Backup and Recovery는 백업에 사용되는 것과 동일한 StorageGRID 또는 ONTAP S3 버킷에 구성 데이터를 저장합니다. 콘솔 에이전트 호스트 시스템에 장애가 발생하면 새 콘솔 에이전트를 배포하고 이 데이터를 복원할 수 있습니다.' 
---
...
[role="lead"]
_프라이빗 모드_(인터넷 액세스 없음)에서 NetApp Backup and Recovery는 백업에 사용되는 것과 동일한 StorageGRID 또는 ONTAP S3 버킷에 구성 데이터를 저장합니다. Console 에이전트 호스트 시스템에 장애가 발생하면 새 Console 에이전트를 배포하고 이 데이터를 복원할 수 있습니다.

클라우드 계정이 관련되지 않은 경우 온프레미스에서 _표준 모드_로 백업 및 복구를 사용할 때 콘솔 에이전트를 교체하고 에이전트에 저장된 구성 데이터를 복원하기 위해 다음 단계를 따를 수도 있습니다(예: NetApp StorageGRID를 백업 대상으로 사용하는 경우).
...
새 Console 에이전트를 설치할 때는 기존 에이전트와 동일한 소프트웨어 버전을 다운로드하십시오. NetApp Backup and Recovery 데이터베이스가 변경되면 최신 소프트웨어 버전이 이전 데이터베이스 백업과 호환되지 않을 수 있습니다.  https://docs.netapp.com/us-en/console-setup-admin/task-upgrade-connector.html["백업 데이터베이스를 복원한 후 콘솔 에이전트 소프트웨어를 최신 버전으로 업그레이드합니다."^].

. https://docs.netapp.com/us-en/console-setup-admin/task-quick-start-private-mode.html["새로운 온프레미스 Linux 호스트에 콘솔 에이전트 설치"^]
. 방금 생성한 관리자 사용자 자격 증명을 사용하여 NetApp Console에 로그인하십시오.
...
. 백업 위치에서 MySQL 백업 파일을 새 Console 에이전트 호스트로 복사합니다. 다음 예에서는 파일 이름 `CBS_DB_Backup_23_05_2023.sql`을 사용합니다.
...
. 인덱싱된 카탈로그 백업 ZIP 파일을 백업 위치에서 새 Console 에이전트 호스트의  `/opt/application/netapp/cbs` 폴더로 복사합니다. 다음 예에서는 파일 이름으로  `Indexed_Catalog_DB_Backup_catalogdb1_23_05_2023.zip`을(를) 사용합니다.
...
----
. *ls* 명령을 실행하여 "catalogdb1" 폴더가 생성되었고 하위 폴더 `files`, `versions` 및 `snapshots`가 포함되어 있는지 확인하십시오.
...
ONTAP 시스템과 연결된 StorageGRID 시스템의 세부 정보를 원래 Console 에이전트 설정에서 구성한 대로 추가하려면  https://docs.netapp.com/us-en/console-automation/index.html["NetApp Console API"^]을 사용하십시오.
...
----
+
IP 주소, 사용자 이름, 비밀번호 값은 사용자 지정 값이지만 계정 이름은 그렇지 않습니다. 계정 이름은 항상  `account-DARKSITE1`입니다. 또한 사용자 이름은 이메일 형식의 이름을 사용해야 합니다.

+
이 API는 다음과 같은 응답을 반환합니다. 아래와 같이 인증 토큰을 확인할 수 있습니다.
...
----
. NetApp Backup and Recovery 데이터베이스를 시스템과 연결된 StorageGRID 시스템의 세부 정보로 업데이트하십시오. 아래와 같이 액세스 키 및 스토리지 키와 함께 StorageGRID의 정규화된 도메인 이름을 입력해야 합니다.
+
[source, shell]
----
...
. 각 ONTAP 시스템을 선택하고 오른쪽 패널에서 백업 및 복구 서비스 옆에 있는 *백업 보기*를 선택하십시오.
+
볼륨에 대해 생성된 모든 백업이 표시되어야 합니다.

. 복원 대시보드의 검색 및 복원 섹션에서 *색인 설정*을 선택합니다.
+
이전에 색인 카탈로그 기능이 활성화되어 있던 시스템은 계속 활성화된 상태로 유지해야 합니다.
