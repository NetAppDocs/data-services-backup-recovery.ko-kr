...
.단계
. NetApp Console 웹 사이트에서 사용자 토큰을 획득합니다.
+
다음 링크에서 새로 고침 토큰을 생성했는지 확인하십시오: https://services.cloud.netapp.com/refresh-token/. 새로 고침 토큰은 사용자 토큰을 생성하는 데 사용되는 영숫자 문자열입니다.
