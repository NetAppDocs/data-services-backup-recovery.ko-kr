...
[role="lead"]
NetApp Console 또는 NetApp Console(로컬 배포) 사용 여부에 따라, 클라우드 공급업체로부터 *NetApp Intelligent Services*에 대한 종량제(PAYGO) 또는 연간 마켓플레이스 구독을 구매하거나 NetApp에서 BYOL(사용자 라이선스 보유) 라이선스를 구매하여 NetApp Backup and Recovery 라이선스를 취득할 수 있습니다. 시스템에서 NetApp Backup and Recovery를 활성화하고, 운영 데이터의 백업을 생성하고, 백업 데이터를 운영 시스템으로 복원하려면 유효한 라이선스가 필요합니다.
...
* NetApp에서 BYOL(Bring-Your-Own-License)이라고도 하는 직접 라이센스를 구매합니다. 이 옵션은 NetApp Console과 Console 로컬 배포 모두에 적용됩니다.
...
NetApp Console 또는 Console 로컬 배포를 사용하여 BYOL 라이선스를 관리할 수 있습니다. 콘솔에서 새 라이선스를 추가하고, 기존 라이선스를 업데이트하고, 라이선스 상태를 확인할 수 있습니다.
...
라이선스 용량을 초과하면 PAYGO 요금이 적용됩니다. PAYGO 구독이 없으면 새 백업을 생성할 수 없지만 기존 백업은 서비스 보증 없이 복원 가능한 상태로 유지됩니다. 라이선스가 만료되기 전에 반드시 갱신하십시오. 만료된 라이선스는 새 백업을 방지하고 서비스를 중단시킵니다.
