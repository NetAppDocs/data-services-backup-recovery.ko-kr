...
[role="lead"]
NetApp Backup and Recovery를 사용하면 기본 온프레미스 ONTAP 시스템의 볼륨 데이터를 ONTAP S3에 백업하고, 선택적으로 복제를 위해 보조 ONTAP 시스템에도 백업할 수 있습니다. 이 페이지를 참조하여 환경을 준비하고 백업을 활성화하십시오.
...
. *로컬 스냅샷*: 기존 스냅샷 정책을 선택하거나 새 정책을 생성합니다.
+
