...
서비스 요금은 NetApp 에 ​​지불되며, 여기에는 개체 스토리지 또는 ONTAP 대상에 대한 백업을 생성하는 비용과 해당 백업에서 볼륨이나 파일을 복원하는 비용이 모두 포함됩니다. 개체 스토리지나 ONTAP 에서 보호하는 데이터에 대해서만 비용을 지불합니다(여기에는 SnapMirror 구성이 포함됩니다). 요금은 ONTAP 볼륨이 개체 스토리지 또는 ONTAP 대상에 백업되는 소스 논리적 사용 용량( ONTAP 효율성 이전)을 사용하여 계산됩니다. 이 용량은 프런트엔드 테라바이트(FETB)라고도 합니다.
