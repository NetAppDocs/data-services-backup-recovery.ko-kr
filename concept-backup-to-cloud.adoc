...
[role="lead"]
이 개요를 통해 NetApp Backup and Recovery가 ONTAP 워크로드를 보호하는 방법, 지원하는 워크로드 및 백업 대상, 라이선스 및 비용에 대해 알아보십시오.
...
ONTAP 시스템에는 백업 및 복구 기능이 기본적으로 내장되어 있으므로 추가 하드웨어나 미디어 게이트웨이가 필요하지 않습니다. 따라서 백업 작업이 간편하고 비용 효율적입니다. NetApp Console을 사용하면 여러 리소스 관리자나 전문 인력 없이도 3-2-1 백업 변형의 전체 스펙트럼을 포함한 모든 백업 전략을 간편하게 구현할 수 있습니다.
...
** VMware 워크로드를 지원되는 스토리지 공급자에 백업합니다.
** 클라우드에서 온프레미스 vCenter로 VMware 데이터를 복원합니다.
** 백업이 수행된 정확한 위치나 대체 위치로 VM을 복원할 수 있습니다.
...
** link:br-use-oracle-protect-overview.html["Oracle Database 워크로드를 보호합니다"]을(를) 참조하세요.
...
** Google Cloud에서는 새 버킷이 배포되며,  https://cloud.google.com/bigquery["Google Cloud BigQuery 서비스"^]계정/프로젝트 수준에서 프로비저닝됩니다.


* 보관 개체 스토리지로 이동된 백업 파일에서 볼륨 데이터를 복원하려는 경우 클라우드 공급자가 GiB당 추가 검색 요금과 요청당 요금을 부과합니다.
* 볼륨 데이터 복원 과정에서 백업 파일을 랜섬웨어 검사할 계획이라면(클라우드 백업에 Datalock 및 Ransomware 보호 기능을 활성화한 경우), 클라우드 공급업체로부터 추가적인 데이터 전송 비용이 발생할 수 있습니다.


*서비스 요금*

ONTAP 볼륨 워크로드의 경우 객체 스토리지로 보호되는 볼륨에 대해서만 요금이 부과됩니다. 요금은 효율성이 적용되기 전 소스 ONTAP 볼륨의 논리적 사용 용량(프런트엔드 테라바이트(FETB)라고도 함)을 기준으로 부과됩니다.

Kubernetes 워크로드의 경우 모든 영구 볼륨의 총 크기를 기준으로 요금이 부과됩니다.

다른 모든 워크로드의 경우, 하나 이상의 보조 스토리지 또는 객체 스토리지 대상으로 보호되는 리소스에 대해 요금이 부과됩니다. 요금은 소스 워크로드의 논리적 크기를 기준으로 계산됩니다. 데이터베이스의 경우 데이터베이스 크기를 의미하고, 가상 머신의 경우 가상 머신 크기를 의미합니다.

백업 및 복구 비용을 지불하는 방법은 세 가지가 있습니다.

* 첫 번째 옵션은 월 단위로 요금을 지불하고 클라우드 제공업체에 가입하는 것입니다.
* 두 번째 옵션은 연간 계약을 구매하는 것입니다.
* 세 번째 옵션은 NetApp에서 직접 라이센스를 구매하는 것입니다. 자세한 내용은 <<라이센스>> 섹션을 참조하십시오.
...
NetApp Backup and Recovery 무료 평가판을 제공하므로 제한된 기간 동안 라이선스 키 없이도 사용할 수 있습니다.

백업 라이선스는 개체 스토리지와 관련된 백업 및 복원 작업에만 필요합니다.  스냅샷과 복제 볼륨을 만드는 데는 라이센스가 필요하지 않습니다.

세 가지 라이선스 옵션 중에서 선택할 수 있습니다.

* *Bring Your Own License(BYOL)*: NetApp 에서 기간 기반(1년, 2년 또는 3년) 및 용량 기반(1TiB 단위) 라이선스를 구매합니다.  제공된 일련번호를 NetApp Console 에 입력하여 활성화하세요.  라이선스는 조직의 모든 소스 시스템을 포함합니다.  기간 또는 용량 한도에 도달하면 갱신이 필요합니다.
* *Pay As You Go(PAYGO)*: 클라우드 공급업체의 마켓플레이스를 통해 구독하고 백업된 데이터의 GiB당 요금을 지불하며, 월 단위로 청구됩니다.  선불은 필요하지 않습니다.  처음 가입하시면 30일 무료 체험이 제공됩니다. 자세한 내용은 다음을 참조하세요. https://docs.netapp.com/us-en/data-services-backup-recovery/br-start-licensing.html#use-a-netapp-backup-and-recovery-paygo-subscription["NetApp Backup and Recovery PAYGO 구독을 사용하세요"] .
* *연간 계약*: AWS 및 Azure 마켓플레이스를 통해 1년, 2년 또는 3년 동안 이용 가능합니다.  연간 계약은 2가지가 있습니다.
+
** *클라우드 백업*: Cloud Volumes ONTAP 및 온프레미스 ONTAP 데이터를 백업합니다.
** *CVO Professional*: Cloud Volumes ONTAP 및 NetApp Backup and Recovery 가 번들로 제공되며, Cloud Volumes ONTAP 볼륨에 대한 무제한 백업이 가능합니다(백업 용량은 라이선스에 포함되지 않음).
+
*** CVO Professional 플랜에는 두 가지 유형의 요금이 있습니다.
+
**** *리소스 요금*: 저장소 사용량에 따라 결정됩니다. 자세한 내용은 다음을 참조하세요. https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/concept-licensing.html#packages["Cloud Volumes ONTAP 라이선싱"] .
**** *서비스 요금*: NetApp Backup and Recovery 에 대한 요금입니다.  하지만 소스 볼륨이 CVO Professional 플랜을 사용하는 스토리지 시스템에 있는 경우 NetApp Backup and Recovery 무료로 제공됩니다.
...
Google Cloud Platform을 사용하는 경우 NetApp 에서 비공개 제안을 요청하고 Google Cloud Marketplace에서 활성화하는 동안 요금제를 선택하세요.
...
== 지원되는 워크로드, 시스템 및 백업 대상

.지원되는 워크로드
NetApp Backup and Recovery 다음과 같은 유형의 워크로드를 보호합니다.

* ONTAP 볼륨
* 물리적 디스크와 VMFS 또는 NFS를 통해 VMware 가상 머신 디스크(VMDK)에 저장된 Microsoft SQL Server 인스턴스 및 데이터베이스
* VMware VM 및 데이터 저장소
* KVM 워크로드
* Hyper-V 워크로드
* Oracle 데이터베이스 워크로드(미리 보기)
* Kubernetes 워크로드


.지원되는 시스템
* 온프레미스 ONTAP SAN(iSCSI 프로토콜) 및 NAS(NFS 및 CIFS 프로토콜 사용)는 ONTAP 버전 9.8 이상이어야 합니다.
* AWS용 Cloud Volumes ONTAP 9.8 이상(SAN 및 NAS 사용)
* Google Cloud Platform용 Cloud Volumes ONTAP 9.8 이상 버전(NFS 및 CIFS 프로토콜 사용)
* Microsoft Azure(SAN 및 NAS 사용)용 Cloud Volumes ONTAP 9.8 이상
* Amazon FSx for NetApp ONTAP (Microsoft SQL Server 워크로드만 해당)


.지원되는 백업 대상
* Microsoft Azure Blob 스토리지
* 아마존 웹 서비스(AWS) S3
* Google Cloud Storage(ONTAP 볼륨 및 Kubernetes 워크로드에만 해당)
* StorageGRID
* ONTAP S3
* S3 호환 스토리지(ONTAP Volumes 워크로드에만 해당)
...
* Azure에서 NetApp Console은 Blob 컨테이너에 대한 스토리지 계정이 있는 새 리소스 그룹 또는 기존 리소스 그룹을 사용합니다. Console은  https://docs.microsoft.com/en-us/azure/storage/blobs/anonymous-read-access-prevent["Blob 데이터에 대한 공개 액세스를 차단합니다."]기본적으로 이 리소스를 사용합니다.
...
== NetApp Backup and Recovery 이용 약관
