...
summary: '사용자 정의 리소스(CR)를 사용하여 Kubernetes 애플리케이션의 온디맨드 스냅샷 및 백업을 생성하고 기준선을 설정하거나 최근 데이터 변경 사항을 보호할 수 있습니다.' 
---
= NetApp Backup and Recovery에서 사용자 지정 리소스를 사용하여 Kubernetes 애플리케이션을 지금 백업하십시오.
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
사용자 정의 리소스(CR)를 사용하여 Kubernetes 애플리케이션의 온디맨드 스냅샷 및 백업을 생성하고 기준선을 설정하거나 최근 데이터 변경 사항을 보호할 수 있습니다.
...
** *spec.reclaimPolicy*: (_선택 사항_) 스냅샷 CR이 삭제될 때 스냅샷의 AppArchive에 어떤 일이 발생하는지 정의합니다. 로 설정하더라도 `Retain`, 스냅샷 CR 자체는 여전히 삭제되며 AppArchive만 유지됩니다. 유효한 옵션:
+
*** `Retain` (기본값)
*** `Delete`
+
...
** *spec.dataMover*: (_선택 사항, Trident Protect에서 마이그레이션된 클러스터의 경우 필수_) 백업 작업에 사용할 백업 도구를 나타내는 문자열입니다. 이 클러스터가 Trident Protect에서 Backup and Recovery로 마이그레이션된 경우 값은 대소문자를 구분하며 `CBS`이어야 합니다.
...
| protect.trident.netapp.io/pvc-bind-timeout-sec | 문자열 | 새로 생성된 PersistentVolumeClaims(PVC)가 작업이 실패하기 전에  `Bound` 단계에 도달할 때까지 기다리는 최대 시간(초)입니다. | "1200" (20분) 
|===