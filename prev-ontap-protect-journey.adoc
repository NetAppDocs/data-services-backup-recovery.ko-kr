...
---
= NetApp Backup and Recovery에서 ONTAP 볼륨 보호 계획
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
이 페이지를 사용하여 NetApp Backup and Recovery로 ONTAP 볼륨을 보호하는 방법을 계획하십시오. 백업 및 복구를 활성화하기 전에 보호 방법, 아키텍처, 정책, 오브젝트 스토리지 옵션 및 Console 에이전트 배포 모드를 검토하십시오.
...
사용할 기능을 선택하기 전에 각 기능의 역할과 제공하는 보호 유형에 대한 간략한 설명을 드리겠습니다.
...
|===
스냅샷은 모든 백업 방법의 기본이며, 백업 및 복구 서비스를 사용하기 위해 필수적입니다. 스냅샷은 볼륨의 읽기 전용 시점 이미지입니다. 이 이미지는 마지막 스냅샷 생성 이후 파일 변경 사항만 기록하므로 스토리지 공간을 최소한으로 사용하고 성능 오버헤드도 거의 없습니다. 그림에서 보는 것처럼, 볼륨에 생성된 스냅샷은 복제된 볼륨과 백업 파일을 소스 볼륨의 변경 사항과 동기화된 상태로 유지하는 데 사용됩니다.

image:diagram-321-overview.png["백업 파일이 소스 시스템에 스냅샷으로, 보조 스토리지 시스템에 복제된 볼륨으로, 개체 스토리지에 백업 파일로 존재하는 방식을 보여주는 다이어그램입니다."]
...
이러한 기준 외에도 클라우드 스토리지는 *DataLock 및 랜섬웨어 보호* 기능을 사용하면 추가적인 보안 옵션을 제공하며, 오래된 백업 파일을 위한 아카이브 스토리지 클래스를 선택하면 추가적인 비용 절감 효과를 얻을 수 있습니다. link:prev-ontap-policy-object-options.html["DataLock 및 랜섬웨어 보호와 보관 스토리지 설정에 대해 자세히 알아보세요."].
...
image:diagram-321-fanout-detailed.png["볼륨의 스냅샷을 사용하여 복제된 볼륨과 백업 파일을 만들고 업데이트하는 방법을 보여주는 다이어그램입니다."]

*캐스케이드* 아키텍처는 스냅샷을 먼저 대상 스토리지 시스템으로 전송한 다음, 해당 시스템이 복사본을 클라우드의 백업 개체로 전송합니다.

image:diagram-321-cascade-detailed.png["볼륨의 스냅샷을 사용하여 복제된 볼륨과 백업 파일을 만들고 업데이트하는 방법을 보여주는 다이어그램입니다."]
...
NetApp에서 제공하는 기본 정책을 사용하여 백업을 생성하거나 사용자 지정 정책을 생성할 수 있습니다. 활성화 마법사를 사용하여 볼륨에 대한 백업 및 복구 서비스를 활성화할 때 기본 정책과 시스템에 이미 존재하는 다른 정책(Cloud Volumes ONTAP 또는 온프레미스 ONTAP 시스템) 중에서 선택할 수 있습니다. 기존 정책과 다른 정책을 사용하려면 활성화 마법사를 시작하기 전이나 마법사를 사용하는 동안 정책을 생성할 수 있습니다.
...
NetApp Backup and Recovery UI에서 스냅샷, 복제 및 오브젝트 스토리지 백업 정책을 생성할 수 있습니다. 자세한 내용은 link:prev-ontap-backup-manage.html["새로운 백업 정책 추가"]을 참조하십시오.

NetApp Backup and Recovery를 사용하여 사용자 지정 정책을 생성하는 것 외에도 System Manager 또는 ONTAP 명령줄 인터페이스(CLI)를 사용할 수 있습니다.
...
백업 정책은 팬아웃 아키텍처 또는 캐스케이드 아키텍처 사용 여부에 따라 다른 위치에 저장됩니다. 복제 정책과 백업 정책은 복제가 두 개의 ONTAP 스토리지 시스템을 쌍으로 연결하는 반면, 오브젝트에 대한 백업은 스토리지 공급자를 대상으로 사용하기 때문에 설계 방식이 다릅니다.
...
| *종속* | 주요한 | 반성 | 반성 
|===
캐스케이드 아키텍처를 사용할 때 사용자 지정 정책을 생성하려면 복제된 볼륨이 생성될 2차 시스템에 복제 정책과 오브젝트 백업 정책을 생성해야 합니다. 팬아웃 아키텍처를 사용할 때 사용자 지정 정책을 생성하려면 복제된 볼륨이 생성될 2차 시스템에 복제 정책을 생성하고 기본 시스템에 오브젝트 백업 정책을 생성해야 합니다.
...
시스템의 오브젝트 스토리지에 백업 파일을 생성하면 기본적으로 백업 및 복구 서비스는 구성된 오브젝트 스토리지 계정에 백업 파일용 컨테이너(버킷 또는 스토리지 계정)를 생성합니다. AWS 또는 GCP 버킷의 기본 이름은 "netapp-backup-<uuid>"입니다. Azure Blob 스토리지 계정의 이름은 "netappbackup<uuid>"입니다.
...
NOTE: Azure에서 백업 대상으로 사용할 컨테이너를 생성할 때 생성하는 컨테이너의 이름이 Azure 스토리지 계정과 동일한지 확인하십시오.
...
이미 콘솔을 사용하여 스토리지를 관리하고 있다면 콘솔 에이전트가 이미 설치되어 있습니다. NetApp Backup and Recovery에서 동일한 콘솔 에이전트를 사용하려는 경우 모든 준비가 완료된 것입니다. 다른 콘솔 에이전트를 사용해야 하는 경우 백업 및 복구 구현을 시작하기 전에 설치하십시오.
...
NetApp Backup and Recovery를 인터넷 연결이 완벽한 사이트(표준 모드 또는 SaaS 모드라고도 함)에서 사용하는 경우 Console에서 관리하는 모든 온프레미스 ONTAP 또는 Cloud Volumes ONTAP 시스템에 복제된 볼륨을 생성할 수 있으며 지원되는 모든 클라우드 공급자의 오브젝트 스토리지에 백업 파일을 생성할 수 있습니다. link:concept-backup-to-cloud.html["지원되는 백업 대상의 전체 목록을 확인하세요."].
...
* 로컬 온프레미스 ONTAP 시스템의 데이터를 로컬 StorageGRID 시스템으로 백업할 수 있습니다. link:prev-ontap-backup-onprem-storagegrid.html["온프레미스 ONTAP 데이터를 StorageGRID 에 백업"].
* 로컬 온프레미스 ONTAP 시스템의 데이터를 S3 개체 스토리지에 대해 구성된 로컬 온프레미스 ONTAP 시스템이나 Cloud Volumes ONTAP 시스템으로 백업할 수 있습니다. link:prev-ontap-backup-onprem-ontaps3.html["온프레미스 ONTAP 데이터를 ONTAP S3에 백업"].
