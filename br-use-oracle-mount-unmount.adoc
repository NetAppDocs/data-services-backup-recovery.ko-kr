...
---
= NetApp Backup and Recovery를 사용하여 Oracle Database 복구 지점을 마운트, 마운트 해제 및 카탈로그화합니다
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
복구 작업을 수행하기 위해 제어된 상태에서 데이터베이스에 액세스해야 하는 경우 Oracle 데이터베이스 복구 지점을 마운트할 수 있습니다. 백업 및 복구 작업 메타데이터를 RMAN 복구 카탈로그에 저장해야 하는데 메타데이터를 자동으로 카탈로그에 추가하는 옵션이 활성화되어 있지 않은 경우 복구 지점을 수동으로 카탈로그에 추가하십시오.



== Oracle 데이터베이스 복구 지점 마운트
...
== Oracle 데이터베이스 복구 지점을 마운트 해제합니다
...
== Oracle 데이터베이스 복구 지점 카탈로그 작성

백업 및 복구 작업 메타데이터를 보존해야 하는 경우 복구 지점을 수동으로 카탈로그화합니다. 메타데이터는 데이터베이스에 대해 선택한 *RMAN 카탈로그 구성* 설정(기본적으로 대상 제어 파일)에 따라 저장됩니다. 연결된 보호 정책에서 자동 카탈로그화가 비활성화된 경우에만 수동으로 카탈로그화해야 합니다.
...
. 목록에서 복구 지점을 선택하고 작업 아이콘 image:../media/icon-action.png["작업 옵션"] > *카탈로그*를 선택합니다.
. *카탈로그*를 선택하여 작업을 확인합니다.
