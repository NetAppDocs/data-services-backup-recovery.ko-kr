...
StorageGRID의 Datalock 및 Ransomware 보호 요구 사항에 대한 자세한 내용은 link:prev-ontap-policy-object-options.html["개체 백업 정책 옵션"]을(를) 참조하십시오.
...
+
클러스터에서 ONTAP 9.11.1 이상을 사용하는 경우 _Datalock and Ransomware protection_을 구성하여 삭제 및 랜섬웨어 공격으로부터 백업을 보호할 수 있습니다. _DataLock_은 백업 파일이 수정되거나 삭제되지 않도록 보호하고, _ransomware protection_은 백업 파일을 검사하여 백업 파일에서 랜섬웨어 공격의 증거를 찾습니다.
