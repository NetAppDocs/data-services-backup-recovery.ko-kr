...
[role="lead"]
NetApp Backup and Recovery에서 몇 가지 단계를 완료하여 기본 온프레미스 ONTAP 시스템의 볼륨 데이터를 백업하세요. 백업 데이터를 보조 ONTAP 스토리지 시스템(복제된 볼륨) 또는 S3 호환 스토리지 시스템의 버킷으로, 또는 둘 다로 보낼 수 있습니다.
