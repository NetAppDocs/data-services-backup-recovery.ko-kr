...
[role="lead"]
NetApp Backup and Recovery의 오브젝트 백업 정책을 사용하여 온프레미스 ONTAP 및 Cloud Volumes ONTAP 시스템에 대한 일정, 보존 기간, DataLock, 랜섬웨어 보호 및 아카이빙 설정을 정의할 수 있습니다.
...
각 백업 정책에는 백업 파일에 적용할 수 있는 _레이블 및 보존_ 섹션이 있습니다. 볼륨에 적용된 스냅샷 정책은 NetApp Backup and Recovery에서 인식하는 정책 중 하나여야 하며, 그렇지 않으면 백업 파일이 생성되지 않습니다.
...
이 두 가지 기능은 백업 파일을 보호하여 랜섬웨어 공격 시도 시에도 항상 유효한 백업 파일에서 데이터를 복구할 수 있도록 합니다. 또한 백업을 잠그고 특정 기간 동안 보관해야 하는 특정 규제 요건을 충족하는 데에도 유용합니다. *DataLock 및 랜섬웨어 보호* 옵션을 활성화하면 NetApp Backup and Recovery 활성화의 일부로 프로비저닝되는 클라우드 버킷에 오브젝트 잠금 및 오브젝트 버전 관리가 활성화됩니다.
...
. 정책을 생성할 때 *DataLock 및 랜섬웨어 보호* 섹션을 확장하십시오.
. 다음 중 하나를 선택하세요.
+
** *없음*: DataLock 보호 및 랜섬웨어 보호 기능이 비활성화되었습니다.
** *잠금 해제됨*: DataLock 보호 및 랜섬웨어 보호 기능이 활성화되었습니다. 특정 권한을 가진 사용자는 보존 기간 동안 보호된 백업 파일을 덮어쓰거나 삭제할 수 있습니다.
** *잠금*: DataLock 보호 및 랜섬웨어 보호 기능이 활성화되었습니다. 보존 기간 동안 사용자는 보호된 백업 파일을 덮어쓰거나 삭제할 수 없습니다. 이는 모든 규정 준수 요건을 충족합니다.
...
| Google Cloud의 Cloud Volumes ONTAP | 구글 클라우드 스토리지 
...
* AWS의 경우:
+
** 클러스터는 ONTAP 9.11.1 이상 버전을 실행해야 합니다.
...
* Azure의 경우:
+
** 클러스터는 ONTAP 9.12.1 이상 버전을 실행해야 합니다.
** 콘솔 에이전트는 클라우드나 사내에 배포될 수 있습니다.
...
또한 DataLock과 관련된 비용에 대한 견적은 다음을 방문하여 얻을 수 있습니다. https://www.netapp.com/data-services/backup-and-recovery-tco-calculator/["NetApp Backup and Recovery 총소유비용(TCO) 계산기"] .
...
AWS, Azure 또는 Google Cloud Storage를 사용하는 경우, 일정 기간이 지난 후 오래된 백업 파일을 더 저렴한 아카이브 스토리지 클래스 또는 액세스 티어로 이동할 수 있습니다. 또한 백업 파일을 표준 클라우드 스토리지에 저장하지 않고 즉시 아카이브 스토리지로 보낼 수도 있습니다. "일 후 아카이브"에 *0*을 입력하면 백업 파일이 아카이브 스토리지로 바로 전송됩니다. 이 기능은 클라우드 백업 데이터에 자주 액세스하지 않는 사용자나 테이프 솔루션을 교체하는 사용자에게 특히 유용합니다.
...
+
** AWS의 경우, AWS _S3 Glacier_ 또는 _S3 Glacier Deep Archive_ 스토리지로 백업을 계층화할 수 있습니다. link:prev-reference-aws-archive-storage-tiers.html["AWS 보관 스토리지에 대해 자세히 알아보세요"^].
** Azure의 경우, 오래된 백업을 Azure Archive 스토리지로 계층화할 수 있습니다. link:prev-reference-azure-archive-storage-tiers.html["Azure 보관 저장소에 대해 자세히 알아보세요"^].
