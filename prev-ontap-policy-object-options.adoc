...
이 두 가지 기능은 백업 파일을 보호하여 랜섬웨어 공격 시도 시에도 항상 유효한 백업 파일에서 데이터를 복구할 수 있도록 합니다. 또한 백업을 잠그고 특정 기간 동안 보관해야 하는 특정 규제 요건을 충족하는 데에도 유용합니다. *Datalock and Ransomware protection* 옵션을 활성화하면 NetApp Backup and Recovery 활성화의 일부로 프로비저닝되는 클라우드 버킷에 객체 잠금 및 객체 버전 관리가 활성화됩니다.
