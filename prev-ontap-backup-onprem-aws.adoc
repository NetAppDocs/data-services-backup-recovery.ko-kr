...
[role="lead"]
NetApp Backup and Recovery를 사용하면 온프레미스 ONTAP 시스템의 볼륨 데이터를 2차 스토리지 시스템과 Amazon S3에 백업할 수 있습니다. 이 페이지를 참조하여 환경을 준비하고 백업을 활성화하십시오.
...
+
"Default"가 아닌 다른 IPspace를 사용하는 경우 오브젝트 스토리지에 액세스하기 위해 정적 라우트를 생성해야 할 수 있습니다.
...
NOTE: 고객 관리 키 암호화는 Amazon FSx for NetApp ONTAP에 대해서만 지원됩니다. Cloud Volumes ONTAP for AWS에서는 지원되지 않습니다.

을 참조하십시오. https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["AWS에서 고객 관리 키 암호화 설정"^]
...
.결과
NetApp Backup and Recovery 볼륨의 초기 백업을 시작합니다. 복제된 볼륨과 백업 파일의 기준 전송에는 기본 스토리지 시스템 데이터의 전체 사본이 포함됩니다. 이후 전송에는 스냅샷에 포함된 기본 데이터의 차등 사본이 포함됩니다.
