...
---
= SnapCenter 와 NetApp Backup and Recovery 간의 정책 차이점
:hardbreaks:
:allow-uri-read: 
:icons: font
:imagesdir: ./media/
...
[role="lead"]
이 참조 자료를 사용하여 가져온 리소스 및 정책에 영향을 미칠 수 있는 SnapCenter 와 NetApp Backup and Recovery 간의 정책 차이점을 이해하십시오.
...
NetApp Backup and Recovery는 다음과 같은 스케줄 계층을 사용하며, 각 계층은 약간씩 다릅니다.
...
* 만약 SnapCenter 일정이 7일을 초과하도록 설정되어 있으면, NetApp Backup and Recovery는 일정을 월간으로 설정합니다. 해당 월 동안 일부 스냅샷이 건너뛰어집니다.
+
...
*예시*: SnapCenter에서 1:00 AM에 시작하여 5시간마다 반복되는 시간별 정책이 있는 경우, 마이그레이션 후 NetApp Backup and Recovery는 일일 5시간 간격(1:00 AM, 6:00 AM, 11:00 AM, 4:00 PM, 9:00 PM)으로 일정을 설정합니다. 일부 시간대는 건너뛰게 됩니다 — 9:00 PM 이후에는 5시간마다 반복하려면 2:00 AM이 되어야 하지만, 항상 1:00 AM이 됩니다.
