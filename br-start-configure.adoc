---
sidebar: sidebar 
permalink: br-start-configure.html 
keywords: backing up, back up, backup, backup application data, Amazon Web services, AWS, NetApp Backup and Recovery, Oracle database, Microsoft SQL Server database, SAP HANA database 
summary: NetApp Backup and Recovery 구성하려면 몇 가지 단계를 완료하세요. 
---
= NetApp Backup and Recovery 설정 구성
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
:linkattrs: 
:imagesdir: ./media/


[role="lead"]
워크로드에 대한 자격 증명을 추가하고, 기존 인프라와 통합하고, 백업 및 복원 작업을 준비하도록 Backup and Recovery 설정을 구성합니다.

* <<호스트 리소스에 대한 자격 증명 추가>> Windows, Microsoft SQL Server, Oracle Database 및 Linux 호스트용 자격 증명으로, NetApp Backup and Recovery가 인증해야 하는 호스트에 사용됩니다. 여기에는 파일이나 폴더를 복원할 때 사용되는 게스트 OS 자격 증명이 포함됩니다.
* <<VMware vCenter 설정 유지>>.
* <<SnapCenter 호스트 리소스 가져오기 및 관리>>. (Microsoft SQL Server 워크로드에만 해당)
* <<KVM 관리 플랫폼을 추가하세요>>. (KVM 워크로드에만 해당)
* <<Windows 호스트의 스냅샷에서 로그 디렉토리 구성>>.
* <<실행 후크 템플릿 만들기>> 백업 작업 전후에 스크립트를 실행합니다. (Kubernetes 워크로드만 해당)
* <<리소스 변환 템플릿을 생성합니다>> 리소스를 복원하면서 수정할 수 있습니다.




[[add-credentials-for-host-resources]]
== 호스트 리소스에 대한 자격 증명 추가

호스트 리소스에 대한 자격 증명을 추가하세요. NetApp Backup and Recovery 이러한 자격 증명을 사용하여 워크로드를 검색하고 백업 정책을 적용합니다.

자격 증명이 없으면 호스트 워크로드에 액세스하고 관리할 수 있는 권한을 부여하여 자격 증명을 만드세요.

다음 유형의 자격 증명을 구성해야 합니다.

* Microsoft SQL Server 자격 증명
* SnapCenter Windows 호스트 자격 증명
* Windows 게스트 OS 자격 증명은 게스트 파일 또는 폴더를 복원할 때 사용됩니다.
* Oracle 데이터베이스 자격 증명
* Linux 호스트 자격 증명


*필수 NetApp Console 역할* 백업 및 복구 슈퍼 관리자. Cloud Volumes ONTAP 클러스터에 저장된 데이터를 보호하는 경우 Cloud Volumes ONTAP 관리자 역할도 필요합니다. 다음에 대해 알아보십시오. link:reference-roles.html["백업 및 복구 역할과 권한"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["모든 서비스에 대한 NetApp Console 액세스 역할에 대해 알아보세요."^].

.단계
. NetApp Backup and Recovery 메뉴에서 *설정*을 선택합니다.
. *자격 증명*의 아래쪽 화살표를 선택하세요.
. *새 자격 증명 추가*를 선택합니다.
. 자격 증명 정보를 입력합니다. 선택한 인증 모드에 따라 다른 필드가 나타납니다. 필드에 대한 자세한 정보를 보려면 정보 *i* 아이콘 위에 마우스를 올려 놓으십시오.
+
** *자격 증명 이름*: 자격 증명의 이름을 입력하세요.
** *인증 모드*: *Windows*, *Microsoft SQL*, *Oracle Database*, 또는 *Linux* 중에서 선택하십시오.
+

TIP: Microsoft SQL Server 워크로드의 경우 Windows 및 Microsoft SQL Server 모두에 대한 자격 증명을 입력해야 하므로 두 세트의 자격 증명이 필요합니다.

+
[role="tabbed-block"]
====
.Windows
--
... *Windows*를 선택한 경우:
+
**** *상담원*: 목록에서 콘솔 상담원을 선택하세요.
**** *도메인 및 사용자 이름*: 자격 증명에 대한 NetBIOS 또는 도메인 FQDN과 사용자 이름을 입력합니다.
**** *비밀번호*: 자격 증명의 비밀번호를 입력하세요.




--
.마이크로소프트 SQL 서버
--
... *Microsoft SQL Server*를 선택하신 경우:
+
**** *도메인 및 사용자 이름*: 자격 증명에 대한 NetBIOS 또는 도메인 FQDN과 사용자 이름을 입력합니다.
**** *비밀번호*: 자격 증명의 비밀번호를 입력하세요.
**** *호스트*: 검색된 SQL Server 호스트 주소를 선택합니다.
**** *SQL Server 인스턴스*: 검색된 SQL Server 인스턴스를 선택합니다.




--
.오라클 데이터베이스
--
... *오라클 데이터베이스*를 선택하신 경우:
+
**** *상담원*: 목록에서 콘솔 상담원을 선택하세요.
**** *사용자 이름*: 자격 증명에 사용할 사용자 이름을 입력하십시오.
**** *비밀번호*: 자격 증명의 비밀번호를 입력하세요.




--
.리눅스
--
... Linux를 선택하신 경우:
+
**** *상담원*: 목록에서 콘솔 상담원을 선택하세요.
**** *사용자 이름*: 자격 증명에 사용할 사용자 이름을 입력하십시오.
**** *비밀번호*: 자격 증명의 비밀번호를 입력하세요.




--
====


. *추가*를 선택하세요.




[[edit-credentials-for-host-resources]]
=== 호스트 리소스에 대한 자격 증명 편집

나중에 생성한 모든 계정 정보의 비밀번호를 수정할 수 있습니다.

*필수 NetApp Console 역할* 백업 및 복구 슈퍼 관리자. Cloud Volumes ONTAP 클러스터에 저장된 데이터를 보호하는 경우 Cloud Volumes ONTAP 관리자 역할도 필요합니다. 다음에 대해 알아보십시오. link:reference-roles.html["백업 및 복구 역할과 권한"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["모든 서비스에 대한 NetApp Console 액세스 역할에 대해 알아보세요."^].

.단계
. NetApp Backup and Recovery 메뉴에서 *설정*을 선택합니다.
. 아래쪽 화살표를 선택하여 *자격 증명* 섹션을 확장합니다.
. 작업 아이콘을 선택하세요image:../media/icon-action.png["작업 옵션"] > *자격 증명 편집*.
+
** *비밀번호*: 자격 증명의 비밀번호를 입력하세요.


. *저장*을 선택하세요.




[[maintain-vmware-vcenter-settings]]
== VMware vCenter 설정 유지

백업할 워크로드를 검색하려면 VMware vCenter Server 자격 증명을 제공하십시오. 자격 증명이 없는 경우 VMware vCenter Server 워크로드에 액세스하고 관리할 수 있는 권한으로 자격 증명을 생성하십시오.

*필수 NetApp Console 역할* 백업 및 복구 슈퍼 관리자. Cloud Volumes ONTAP 클러스터에 저장된 데이터를 보호하는 경우 Cloud Volumes ONTAP 관리자 역할도 필요합니다. 다음에 대해 알아보십시오. link:reference-roles.html["백업 및 복구 역할과 권한"]. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["모든 서비스에 대한 NetApp Console 액세스 역할에 대해 알아보세요."^].

.단계
. NetApp Backup and Recovery 메뉴에서 *설정*을 선택합니다.
. 아래쪽 화살표를 선택하여 *VMware vCenter* 섹션을 확장합니다.
. *vCenter 추가*를 선택합니다.
. VMware vCenter Server 정보를 입력합니다.
+
** *vCenter FQDN 또는 IP 주소*: VMware vCenter 서버의 FQDN 또는 IP 주소를 입력하십시오.
** *사용자 이름* 및 *비밀번호*: VMware vCenter Server의 사용자 이름과 비밀번호를 입력하세요.
** *포트*: VMware vCenter Server의 포트 번호를 입력하세요.
** *프로토콜*: *HTTP* 또는 *HTTPS*를 선택하세요.


. *추가*를 선택하세요.




[[import-and-manage-snapcenter-host-resources]]
== SnapCenter 호스트 리소스 가져오기 및 관리

이전에 SnapCenter 사용하여 리소스를 백업한 경우 NetApp Backup and Recovery 에서 해당 리소스를 가져와 관리할 수 있습니다. 이 옵션을 사용하면 SnapCenter 서버 정보를 가져와서 여러 SnapCenter 서버를 등록하고 데이터베이스 작업 부하를 검색할 수 있습니다.

이는 두 부분으로 구성된 프로세스입니다.

* SnapCenter 서버 애플리케이션 및 호스트 리소스 가져오기
* 선택한 SnapCenter 호스트 리소스 관리


*필수 NetApp Console 역할* Backup and Recovery super admin 또는 Backup and Recovery 백업 관리자. Cloud Volumes ONTAP 클러스터에 저장된 데이터를 보호하는 경우, Cloud Volumes ONTAP Admin 역할도 필요합니다. link:reference-roles.html["백업 및 복구 역할과 권한"]에 대해 자세히 알아보십시오. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["모든 서비스에 대한 NetApp Console 액세스 역할에 대해 알아보세요."^].



[[import-snapcenter-server-application-and-host-resources]]
=== SnapCenter 서버 애플리케이션 및 호스트 리소스 가져오기

첫 번째 단계에서는 SnapCenter 에서 호스트 리소스를 가져와 NetApp Backup and Recovery 인벤토리 페이지에 해당 리소스를 표시합니다.  그 시점에서는 리소스가 아직 NetApp Backup and Recovery 에서 관리되지 않습니다.


TIP: SnapCenter 호스트 리소스를 가져온 후에는 NetApp Backup and Recovery 보호 관리를 수행하지 않습니다.  그렇게 하려면 NetApp Backup and Recovery 에서 이러한 리소스를 관리하도록 명시적으로 선택해야 합니다.

.단계
. NetApp Backup and Recovery 메뉴에서 *설정*을 선택합니다.
. 아래쪽 화살표를 선택하여 * SnapCenter 에서 가져오기* 섹션을 확장합니다.
. SnapCenter 리소스를 가져오려면 * SnapCenter 에서 가져오기*를 선택하세요.
. * SnapCenter 애플리케이션 자격 증명*을 입력하세요:
+
.. * SnapCenter FQDN 또는 IP 주소*: SnapCenter 애플리케이션 자체의 FQDN 또는 IP 주소를 입력하세요.
.. *포트*: SnapCenter 서버의 포트 번호를 입력하세요.
.. *사용자 이름* 및 *비밀번호*: SnapCenter 서버의 사용자 이름과 비밀번호를 입력하세요.
.. *콘솔 에이전트*: SnapCenter 의 콘솔 에이전트를 선택하세요.


. * SnapCenter 서버 호스트 자격 증명*을 입력하세요:
+
.. *기존 자격 증명*: 이 옵션을 선택하면 이미 추가한 기존 자격 증명을 사용할 수 있습니다.  자격 증명 이름을 입력하세요.
.. *새로운 자격 증명 추가*: 기존 SnapCenter 호스트 자격 증명이 없으면 새 자격 증명을 추가할 수 있습니다. 자격 증명 이름, 인증 모드, 사용자 이름 및 비밀번호를 입력하세요.


. *가져오기*를 선택하여 항목을 검증하고 SnapCenter 서버를 등록합니다.
+

NOTE: SnapCenter Server가 이미 등록되어 있는 경우 기존 등록 정보를 업데이트할 수 있습니다.



.결과
인벤토리 페이지에는 가져온 SnapCenter 리소스가 표시됩니다.



[[manage-snapcenter-host-resources]]
=== SnapCenter 호스트 리소스 관리

SnapCenter 리소스를 가져온 후 NetApp Backup and Recovery 에서 해당 호스트 리소스를 관리합니다.  가져온 리소스를 관리하도록 선택하면 NetApp Backup and Recovery SnapCenter 에서 가져온 리소스를 백업하고 복구할 수 있습니다.  더 이상 SnapCenter Server에서 해당 리소스를 관리할 필요가 없습니다.

.단계
. SnapCenter 리소스를 가져온 후 나타나는 인벤토리 페이지에서 NetApp Backup and Recovery 에서 관리하려는 가져온 SnapCenter 리소스를 선택합니다.
. 작업 아이콘을 선택하세요image:../media/icon-action.png["작업 옵션"] > *관리*: 리소스를 관리합니다.
. * NetApp Console 에서 관리*를 선택합니다.
+
인벤토리 페이지에서는 호스트 이름 아래에 *관리됨*이 표시되어 선택한 호스트 리소스가 이제 NetApp Backup and Recovery 에서 관리된다는 것을 나타냅니다.





[[edit-imported-snapcenter-resources]]
=== 가져온 SnapCenter 리소스 편집

나중에 SnapCenter 리소스를 다시 가져오거나 가져온 SnapCenter 리소스를 편집하여 등록 정보를 업데이트할 수 있습니다.

SnapCenter 서버의 포트와 비밀번호 세부 정보만 변경할 수 있습니다.

.단계
. NetApp Backup and Recovery 메뉴에서 *설정*을 선택합니다.
. * SnapCenter 에서 가져오기*를 위해 아래쪽 화살표를 선택하세요.
+
SnapCenter 에서 가져오기 페이지에는 이전에 가져온 모든 항목이 표시됩니다.

. 작업 아이콘을 선택하세요image:../media/icon-action.png["작업 옵션"] > *편집*하여 리소스를 업데이트합니다.
. 필요에 따라 SnapCenter 비밀번호와 포트 세부 정보를 업데이트하세요.
. *가져오기*를 선택하세요.




[[add-a-kvm-management-platform]]
== KVM 관리 플랫폼을 추가하세요

Apache CloudStack 관리 플랫폼을 사용하여 KVM 리소스를 관리하는 경우, NetApp Backup and Recovery와 통합하여 Backup and Recovery가 관리되는 KVM 호스트 및 VM을 검색하고 보호할 수 있도록 하십시오.

*필수 NetApp Console 역할* Backup and Recovery super admin 또는 Backup and Recovery 백업 관리자. Cloud Volumes ONTAP 클러스터에 저장된 데이터를 보호하는 경우, Cloud Volumes ONTAP Admin 역할도 필요합니다. link:reference-roles.html["백업 및 복구 역할과 권한"]에 대해 자세히 알아보십시오. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["모든 서비스에 대한 NetApp Console 액세스 역할에 대해 알아보세요."^].

.단계
. NetApp Backup and Recovery 메뉴에서 *설정*을 선택합니다.
. 아래쪽 화살표를 선택하여 *관리 플랫폼* 섹션을 확장하세요.
. *관리 플랫폼 자격 증명 추가*를 선택하세요.
. 다음 정보를 입력하세요:
+
** *관리 플랫폼 IP 주소 또는 FQDN*: 관리 플랫폼의 IP 주소 또는 FQDN을 입력하십시오.
** *API 키*: API 요청 인증에 사용할 API 키를 입력하세요.
** *비밀 키*: API 요청 인증에 사용할 비밀 키를 입력하세요.
** *포트*: 백업 및 복구와 관리 플랫폼 간의 통신에 사용할 포트를 입력하십시오.
** *에이전트*: 백업 및 복구와 관리 플랫폼 간의 통신을 원활하게 하기 위해 사용할 콘솔 에이전트를 선택하십시오.


. 완료되면 *추가*를 선택하세요.




[[configure-log-directories-in-snapshots-for-windows-hosts]]
== Windows 호스트의 스냅샷에서 로그 디렉토리 구성

Windows 호스트용 정책을 생성하기 전에 백업 프로세스 중에 생성된 로그를 저장하기 위해 스냅샷에서 로그 디렉토리를 구성하십시오.

*필수 NetApp Console 역할* 백업 및 복구 슈퍼 관리자, 백업 및 복구 백업 관리자 또는 백업 및 복구 복원 관리자. Cloud Volumes ONTAP 클러스터에 저장된 데이터를 보호하는 경우 Cloud Volumes ONTAP 관리자 역할도 필요합니다. 자세한 내용은 link:reference-roles.html["백업 및 복구 역할과 권한"]을 참조하십시오. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["모든 서비스에 대한 NetApp Console 액세스 역할에 대해 알아보세요."^]

.단계
. NetApp Backup and Recovery 메뉴에서 *인벤토리*를 선택합니다.
. 인벤토리 페이지에서 작업 부하를 선택한 다음 작업 아이콘을 선택합니다.image:../media/icon-action.png["작업 옵션"] > *세부정보 보기*를 클릭하면 작업 부하 세부 정보가 표시됩니다.
. Microsoft SQL Server가 표시된 인벤토리 세부 정보 페이지에서 호스트 탭을 선택합니다.
. 인벤토리 세부 정보 페이지에서 호스트를 선택하고 작업 아이콘을 선택합니다.image:../media/icon-action.png["작업 옵션"] > *로그 디렉토리 구성*.
. 로그 디렉토리의 경로를 찾아보거나 입력하세요.
. *저장*을 선택하세요.




[[create-an-execution-hook-template]]
== 실행 후크 템플릿 만들기

애플리케이션에서 데이터 보호 작업 전이나 후에 작업을 수행하는 데 사용할 수 있는 사용자 정의 실행 후크 템플릿을 만들 수 있습니다.


NOTE: 여기에서 생성하는 템플릿은 Kubernetes 워크로드를 보호할 때만 사용할 수 있습니다.

.단계
. 콘솔에서 *보호* > *백업 및 복구*로 이동합니다.
. *설정* 탭을 선택하세요.
. *실행 후크 템플릿* 섹션을 확장합니다.
. *실행 후크 템플릿 만들기*를 선택합니다.
. 실행 후크의 이름을 입력하세요.
. 원하는 후크 유형을 선택하세요. 예를 들어, 복원 후 후크는 복원 작업이 완료된 후 실행됩니다.
. *스크립트* 텍스트 상자에 실행 후크 템플릿의 일부로 실행하려는 실행 가능한 셸 스크립트를 입력합니다.  선택적으로 *스크립트 업로드*를 선택하여 스크립트 파일을 업로드할 수 있습니다.
. *만들기*를 선택하세요.
+
템플릿을 생성하면 *실행 후크 템플릿* 섹션의 템플릿 목록에 나타납니다.





[[create-a-resource-transformation-template]]
== 리소스 변환 템플릿을 생성합니다

복원 작업 중에 리소스에 적용하는 일반적인 변환을 저장하기 위해 리소스 변환 템플릿을 생성합니다.

*필수 NetApp Console 역할* 백업 및 복구 슈퍼 관리자, 백업 및 복구 백업 관리자 또는 백업 및 복구 복원 관리자. Cloud Volumes ONTAP 클러스터에 저장된 데이터를 보호하는 경우 Cloud Volumes ONTAP 관리자 역할도 필요합니다. 자세한 내용은 link:reference-roles.html["백업 및 복구 역할과 권한"]을 참조하십시오. https://docs.netapp.com/us-en/console-setup-admin/reference-iam-predefined-roles.html["모든 서비스에 대한 NetApp Console 액세스 역할에 대해 알아보세요."^]

.단계
. NetApp Backup and Recovery 메뉴에서 *설정*을 선택합니다.
. 아래쪽 화살표를 선택하여 *리소스 변환 템플릿* 섹션을 확장합니다.
. *템플릿 만들기*를 선택합니다.
. 리소스 변환 템플릿의 이름을 입력합니다.
. 선택적으로 템플릿 설명을 입력합니다.
. 대상 리소스를 설명하십시오.
+
.. 리소스 종류(예: Pod 또는 Deployment)를 입력하세요. 이 필드는 필수입니다.
.. 선택적으로 리소스 그룹, 버전 및 이름을 입력할 수 있습니다.


. 작업 구축:
+
.. *작업* 목록에서 작업을 선택합니다.
.. 작업이 적용되는 JSON 경로를 입력합니다.
.. 해당하는 경우 추가 또는 업데이트할 값이나 이동할 JSON 경로를 입력합니다.
+
이러한 필드는 필드가 필요한 작업에만 나타납니다.

.. 이 템플릿에 다른 작업을 추가하려면 *작업 추가*를 선택하세요.


. 완료되면 *추가*를 선택하세요.

