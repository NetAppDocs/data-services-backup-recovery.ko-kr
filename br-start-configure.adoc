...
* <<호스트 리소스에 대한 자격 증명 추가>> NetApp Backup and Recovery가 인증해야 하는 모든 Windows, Microsoft SQL Server, Oracle Database 또는 Linux 호스트에 적용됩니다. 여기에는 파일이나 폴더를 복원할 때 사용되는 게스트 OS 자격 증명이 포함됩니다.
...
리소스를 복원할 때 수행하는 일반적인 변환을 저장하기 위해 리소스 변환 템플릿을 생성하십시오.

.단계
. NetApp Backup and Recovery 메뉴에서 *설정*을 선택합니다.
. 아래쪽 화살표를 선택하여 *Resource modifer template* 섹션을 확장하세요.
. *템플릿 만들기*를 선택합니다.
