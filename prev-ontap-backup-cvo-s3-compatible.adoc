...
---
= NetApp Backup and Recovery를 사용하여 Cloud Volumes ONTAP 데이터를 S3 호환 스토리지에 백업하세요
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup and Recovery에서 몇 가지 단계를 완료하여 Cloud Volumes ONTAP 시스템의 볼륨 데이터를 S3에 백업하는 방법을 알아보세요.
...
S3 호환 스토리지에 볼륨 백업을 시작하기 전에 지원되는 구성인지 확인하려면 다음 요구 사항을 읽어보세요.

다음 이미지는 각 구성 요소와 구성 요소 간에 준비해야 할 연결을 보여줍니다.

선택적으로, 공용 또는 개인 연결을 사용하여 복제된 볼륨의 보조 ONTAP 시스템에 연결할 수도 있습니다.

image:diagram_cloud_backup_cvo_aws.png["NetApp Backup and Recovery 백업 파일이 있는 소스 시스템의 볼륨과 대상 스토리지와 통신하는 방식을 보여주는 다이어그램입니다."]
...
지원되는 ONTAP 버전:: 최소 ONTAP 버전 9.17.1이 필요합니다.
데이터 암호화를 위한 고객 관리 키 사용에 필요한 정보:: 활성화 마법사에서 기본 S3 암호화 키 대신 고객 관리 키를 선택하여 데이터를 암호화할 수 있습니다. 이 경우 암호화 관리 키가 이미 설정되어 있어야 합니다. https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["자신의 키를 사용하는 방법을 확인하세요"^].
...
다음은 S3 호환 스토리지에 백업할 때 지원되는 작업입니다.

* 온프레미스 ONTAP에서 타사 S3 호환 오브젝트 스토리지로 크래시 정합성 보장 및 애플리케이션 정합성 보장 스냅샷 백업
* 핵심 데이터 보호 워크플로: 백업, 복원 및 백업 삭제
* Wasabi 객체 스토리지로 테스트 및 검증을 완료했으며, MinIO 및 IBM Cloud Object Storage를 사용한 성공적인 구축 사례도 보고되었습니다.
* NetApp Support는 타사 S3 호환 객체 저장소와 관련된 모든 지원 요청을 접수하고 우선순위를 정합니다.
...
S3 호환 스토리지에 백업할 때 다음 기능 및 구성은 지원되지 않습니다.

* BYOB(Bring Your Own Bucket)는 지원되지 않습니다
* 백업 정책에서 DataLock 활성화는 지원되지 않습니다.
* 백업 정책에서 아카이빙 모드 활성화는 지원되지 않습니다.
* 버킷 수준 암호화 옵션은 지원되지 않습니다.
* MetroCluster 구성은 지원되지 않습니다
* 클라우드 기반 ONTAP 환경은 지원되지 않으며 온프레미스 ONTAP만 지원됩니다
...
S3 호환 스토리지에 백업할 때 다음 사항을 유념하십시오.

* *성능이 보장되지 않습니다*: NetApp 제품은 S3 API 지연 시간 및 IOPS에 민감합니다. 유사한 성능을 얻으려면 타사 객체 스토어가 AWS S3와 동등한 지연 시간 및 IOPS를 제공해야 합니다. NetApp은 객체 스토어 성능 문제를 해결하지 않습니다.
* *데이터 손상 또는 손실에 대한 책임 없음*: NetApp은 타사 S3 객체 저장소 문제로 인한 데이터 손실 또는 서비스 중단에 대해 책임을 지지 않습니다.
* *제한된 기능 지원*: 타사 S3 호환 대상에 대해 지원되는 기능 세트는 네이티브 대상(AWS S3, Azure Blob, GCP, ONTAP S3, StorageGRID)에서 사용할 수 있는 기능의 하위 집합입니다.
...
NetApp Backup and Recovery PAYGO 라이선스의 경우 AWS Marketplace에서 Cloud Volumes ONTAP 및 NetApp Backup and Recovery 배포할 수 있는 콘솔 구독을 이용할 수 있습니다.  당신은 필요합니다 https://aws.amazon.com/marketplace/pp/prodview-oorxakq6lq7m4?sr=0-8&ref_=beagle&applicationId=AWSMPContessa["이 NetApp Console 구독을 구독하세요"^] NetApp Backup and Recovery 활성화하기 전에.  NetApp Backup and Recovery 대한 청구는 이 구독을 통해 이루어집니다.
...
Cloud Volumes ONTAP 과 NetApp Backup and Recovery 묶을 수 있는 연간 계약의 경우 Cloud Volumes ONTAP 시스템을 생성할 때 연간 계약을 설정해야 합니다.  이 옵션을 사용하면 온프레미스 데이터를 백업할 수 없습니다.
...
콘솔 에이전트는 전체 또는 제한된 인터넷 액세스("표준" 또는 "제한" 모드)가 가능한 AWS 지역에 설치해야 합니다. https://docs.netapp.com/us-en/console-setup-admin/concept-modes.html["자세한 내용은 NetApp Console 배포 모드를 참조하세요."^] .
...
필수 AWS Cloud Volumes ONTAP 권한:: Cloud Volumes ONTAP 시스템이 ONTAP 9.12.1 이상 소프트웨어를 실행하는 경우, 해당 시스템에 권한을 제공하는 IAM 역할에는 최신 https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-set-up-iam-roles.html["Cloud Volumes ONTAP 정책"^]의 NetApp Backup and Recovery에 특화된 새로운 S3 호환 권한 세트가 포함되어야 합니다.
+
--
콘솔 버전 3.9.23 이상을 사용하여 Cloud Volumes ONTAP 시스템을 만든 경우 이러한 권한은 이미 IAM 역할의 일부여야 합니다.  그렇지 않으면 누락된 권한을 추가해야 합니다.

--
지원되는 AWS 지역:: NetApp Backup and Recovery AWS GovCloud 지역을 포함한 모든 AWS 지역에서 지원됩니다.
...
+
백업을 위한 S3 호환 대상이 *시스템* 페이지에 클러스터로 존재하는 경우, 해당 클러스터를 S3 호환 시스템으로 드래그하여 설정 마법사를 시작할 수 있습니다.
...
** 백업 및 복구 메뉴에서 *볼륨*을 선택하세요. 볼륨 탭에서 *작업*을 선택합니다. image:icon-action.png["작업 아이콘"] 아이콘 옵션을 선택하고 (복제 또는 객체 스토리지 백업이 이미 활성화되지 않은) 단일 볼륨에 대해 *3-2-1 보호 활성화*를 선택합니다.
...
* 보호할 소스 볼륨을 선택할 때 SnapLock 설정이 일치하는 볼륨만 선택할 수 있습니다. 선택하는 볼륨은 모두 SnapLock 기능이 활성화되어 있거나 모두 SnapLock 기능이 비활성화되어 있어야 합니다.
...
** *암호화*: 새 버킷을 생성한 경우 공급자로부터 받은 암호화 키 정보를 입력하십시오. 데이터 암호화를 관리할 때 기본 AWS 암호화 키를 사용할지, 아니면 AWS 계정에서 직접 관리하는 고객 키를 사용할지 선택하세요. (https://docs.netapp.com/us-en/storage-management-cloud-volumes-ontap/task-setting-up-kms.html["자신의 암호화 키를 사용하는 방법을 확인하세요"^]).
+
...
... 정책의 이름을 입력하세요.
... 일반적으로 서로 다른 빈도로 최대 5개의 일정을 선택하세요.
... 객체 백업 정책의 경우 Datalock 및 Ransomware 보호 설정을 지정하십시오. Datalock 및 Ransomware 보호에 대한 자세한 내용은 link:prev-ontap-policy-object-options.html["개체 백업 정책 설정"]를 참조하십시오.
... *만들기*를 선택하세요.
...
. 선택적으로 *로컬 스냅샷, 복제 및 백업에서 일치하지 않는 레이블을 자동으로 수정*할 수 있습니다. 이렇게 하면 스냅샷, 복제 및 백업 정책의 레이블과 일치하는 레이블이 지정된 스냅샷이 생성됩니다.
. *백업 활성화*를 선택하세요.


.결과
NetApp Backup and Recovery 볼륨의 초기 백업을 시작합니다. 복제된 볼륨과 백업 파일의 기준 전송에는 기본 스토리지 시스템 데이터의 전체 사본이 포함됩니다. 이후 전송에는 스냅샷에 포함된 기본 스토리지 시스템 데이터의 차등 사본이 포함됩니다.
