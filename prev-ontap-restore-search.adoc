...
summary: 검색 및 복원 기능을 사용하여 ONTAP 백업 파일에서 볼륨, 폴더 또는 파일을 복구할 수 있습니다. 
---
= NetApp Backup and Recovery의 Search and Restore를 사용하여 ONTAP 데이터 복구
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
ONTAP 시스템, 볼륨 또는 파일 이름을 정확히 모르는 경우 검색 및 복원 기능을 사용하여 ONTAP 백업에서 볼륨, 폴더 또는 파일을 복구할 수 있습니다. 로컬 스냅샷, 복제된 볼륨 및 오브젝트 스토리지 백업에서 검색할 수 있습니다.
...
link:prev-reference-aws-archive-storage-tiers.html["AWS 보관 스토리지에서 복원하는 방법에 대해 자세히 알아보세요."]. link:prev-reference-azure-archive-storage-tiers.html["Azure 보관 저장소에서 복원하는 방법에 대해 자세히 알아보세요."]. link:prev-reference-gcp-archive-storage-tiers.html["Google 보관 저장소에서 복원하는 방법에 대해 자세히 알아보세요."].
...
* SnapLock이 활성화된 볼륨을 복원할 때, 복원된 볼륨은 대상 스토리지가 WORM을 지원하는 경우에만 SnapLock 보호 기능을 유지합니다. WORM을 지원하지 않는 대상으로 복원하는 경우, 복원된 볼륨에서 SnapLock 상태가 제거됩니다.


====
시작하기 전에 복원하려는 볼륨이나 파일의 이름 또는 위치를 어느 정도 알고 있어야 합니다.
...
| StorageGRID | 온프레미스 ONTAP 시스템 Cloud Volumes ONTAP | 온프레미스 ONTAP 시스템 
...
* Amazon S3의 경우 콘솔 에이전트는 AWS 또는 온프레미스에 배포할 수 있습니다.
* Azure Blob의 경우 콘솔 에이전트는 Azure 또는 온프레미스에 배포할 수 있습니다.
* Google Cloud Storage의 경우 콘솔 에이전트는 Google Cloud Platform VPC에 배포되어야 합니다.
* StorageGRID의 경우 Console 에이전트는 인터넷 액세스 여부에 관계없이 온프레미스에 배포해야 합니다.
* ONTAP S3의 경우 Console 에이전트는 온프레미스(인터넷 액세스 여부와 관계없이) 또는 클라우드 공급자 환경에 배포할 수 있습니다.


"온프레미스 ONTAP 시스템"이라는 용어에는 FAS, AFF 및 ONTAP Select 시스템이 포함된다는 점에 유의하십시오.
...
** 스토리지 VM에 IPv4 NFS 데이터 LIF를 구성하고 볼륨에 대한 NFS 내보내기를 설정해야 합니다. Search & Restore는 NFS 및 SMB/CIFS 볼륨을 모두 지원합니다.
** SnapDiff RPC 서버는 스토리지 VM에서 활성화해야 합니다. 콘솔은 시스템에서 인덱싱을 활성화할 때 이 작업을 자동으로 수행합니다. (SnapDiff는 스냅샷 간의 파일 및 디렉터리 차이점을 신속하게 식별하는 기술입니다.)
...
* Azure 요구 사항:
+
** 구독에 Azure Synapse Analytics 리소스 공급자("Microsoft.Synapse"라고 함)를 등록해야 합니다. https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/resource-providers-and-types#register-resource-provider["구독을 위해 이 리소스 공급자를 등록하는 방법을 확인하세요."^]. 리소스 공급자를 등록하려면 구독 *소유자* 또는 *기여자*여야 합니다.
...
NOTE: Backup and Recovery는 동일 볼륨의 다른 스냅샷과 생성 타임스탬프가 정확히 일치하는 스냅샷은 인덱싱하지 않습니다.
...
먼저 카탈로그 파일을 저장할 별도의 볼륨을 마운트하십시오. 이렇게 하면 스냅샷 카탈로그 파일의 크기가 너무 커지는 경우 데이터 손실을 방지할 수 있습니다. 모든 클러스터에서 이 작업이 필요한 것은 아니며, 환경 내 어느 클러스터에서든 원하는 볼륨을 마운트할 수 있습니다. 이 작업을 수행하지 않으면 인덱싱이 제대로 작동하지 않을 수 있습니다.
...
. 복원 대시보드를 표시하려면 *복원* 탭을 선택합니다.
. _검색 및 복원_ 섹션에서 *검색 및 복원*을 선택합니다.
. 검색 및 복원 페이지에서:
+
.. _검색 창_에 볼륨 이름 전체 또는 일부, 폴더 이름 또는 파일 이름을 입력합니다.
.. 리소스 유형을 선택하세요: *볼륨*, *파일*, *폴더*, *모두*.
.. _필터 기준_ 영역에서 필터 조건을 선택합니다. 예를 들어 .JPEG 파일 등 데이터가 저장된 시스템과 파일 형식을 선택할 수 있습니다. 또한 사용 가능한 스냅샷 또는 오브젝트 스토리지의 백업 파일 내에서만 검색하려면 백업 위치 유형을 선택할 수도 있습니다.
...
+
검색 결과에는 검색한 파일이 포함된 로컬 볼륨 스냅샷과 원격 복제 볼륨이 표시됩니다. 클라우드 백업 파일, 스냅샷 또는 복제 볼륨 중에서 복원할 위치를 선택할 수 있습니다.
...
+
*** Amazon S3에서 복원할 때는 대상 볼륨이 위치할 ONTAP 클러스터의 IPspace를 선택하고, ONTAP 클러스터가 S3 버킷에 액세스할 수 있도록 생성한 사용자의 액세스 키와 비밀 키를 입력합니다. 또한, 선택적으로 안전한 데이터 전송을 위해 프라이빗 VPC 엔드포인트를 선택할 수 있습니다. 프라이빗 VPC 엔드포인트를 사용하는 경우, 구성된 프록시 연결보다 우선합니다. link:prev-ontap-backup-onprem-aws.html["이러한 요구 사항에 대한 세부 정보를 확인하세요"].
