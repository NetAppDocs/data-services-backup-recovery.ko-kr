...
---
= NetApp Backup and Recovery에서 Kubernetes 애플리케이션 추가 및 보호
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
NetApp Backup and Recovery 웹 UI 또는 사용자 지정 리소스 파일을 사용하여 보호할 Kubernetes 애플리케이션을 추가할 수 있습니다. 표준 Kubernetes 리소스를 사용하는 네임스페이스 기반 애플리케이션 또는 하나 이상의 가상 머신을 사용하는 VM 기반 애플리케이션을 추가할 수 있습니다.
...
Kubernetes 애플리케이션을 보호하려면 먼저 NetApp Backup and Recovery에서 애플리케이션을 생성해야 합니다. 애플리케이션을 생성하면 NetApp Backup and Recovery가 Kubernetes 클러스터에서 실행 중인 애플리케이션을 인식하게 됩니다.
...
. 페이지 오른쪽 상단에서 워크로드 목록에 *Kubernetes*가 선택되어 있는지 확인하십시오.
. Kubernetes 리소스를 보려면 워크로드 항목에서 *보기*를 선택하십시오.
. *응용 프로그램* 탭을 선택하세요.
. *애플리케이션 만들기*를 선택하세요.
. 애플리케이션의 이름을 입력하세요.
. *Cluster* 목록에서 애플리케이션이 호스팅되는 클러스터를 선택합니다.
. *필터*에서 *네임스페이스*를 선택하여 네임스페이스별로 애플리케이션을 필터링합니다.
...
** 라벨 선택기
+
... 클러스터 수준으로 범위가 지정된 리소스를 추가하려면 *클러스터 범위 리소스 추가*를 선택하십시오. 이러한 리소스를 포함하면 애플리케이션을 생성할 때 애플리케이션에 추가됩니다.
... 선택적으로 *검색*을 선택하여 검색 기준에 따라 리소스를 찾으세요.
+

NOTE: NetApp Backup and Recovery는 검색 매개변수나 결과를 저장하지 않습니다. 이러한 매개변수는 선택된 Kubernetes 클러스터에서 애플리케이션에 포함될 수 있는 리소스를 검색하는 데 사용됩니다.





. Backup and Recovery에 검색 조건과 일치하는 리소스 목록이 표시됩니다.
. 보호하려는 리소스가 목록에 포함되어 있으면 *다음*을 선택합니다.
. 선택적으로 *Policy* 영역에서 기존 보호 정책을 선택하여 애플리케이션을 보호하거나 새 정책을 생성할 수 있습니다. 정책을 선택하지 않으면 애플리케이션에 보호 정책이 적용되지 않은 상태로 생성됩니다. link:br-use-policies-kubernetes-create.html#create-a-policy["보호 정책 추가"] 나중에 정책을 추가할 수 있습니다.
...
. 애플리케이션의 이름을 입력하세요.
. *Cluster* 목록에서 애플리케이션이 호스팅되는 클러스터를 선택합니다.
. *필터*에서 *가상 머신*을 선택하여 VM 기반 애플리케이션을 생성합니다.
. 네임스페이스를 선택하고 필요에 따라 레이블 선택기를 포함하여 애플리케이션에 추가할 가상 머신을 찾습니다.
+

NOTE: 목록에서 VM을 선택하면 애플리케이션 정의가 고정됩니다. 즉, 나중에 새 VM을 애플리케이션에 추가할 수 없습니다(VM을 추가하고 보호하려면 애플리케이션을 편집해야 합니다). 레이블 선택기를 사용하는 경우 개별 VM을 선택하거나 생성된 목록을 편집할 수는 없지만, 나중에 선택기와 일치하는 모든 VM은 자동으로 포함되고 보호됩니다.

+
선택한 가상 머신이 오른쪽 목록에 나타납니다.

. 보호하려는 VM이 목록에 포함되어 있으면 *다음*을 선택하십시오.
. 선택적으로 *Policy* 영역에서 기존 보호 정책을 선택하여 애플리케이션을 보호하거나 새 정책을 생성할 수 있습니다. 정책을 선택하지 않으면 애플리케이션에 보호 정책이 적용되지 않은 상태로 생성됩니다. link:br-use-policies-kubernetes-create.html#create-a-policy["보호 정책 추가"] 나중에 정책을 추가할 수 있습니다.
. *Prescripts and postscripts* 영역에서 백업 작업 전이나 후에 실행하려는 Prescripts 또는 Postscripts 실행 후크를 활성화하고 구성합니다.  사전 스크립트 또는 사후 스크립트를 활성화하려면 이미 하나 이상을 만들어야 합니다.link:br-use-manage-execution-hook-templates.html["실행 후크 템플릿"] .
. *만들기*를 선택하세요.
...
.결과
애플리케이션이 생성되면 Kubernetes 인벤토리의 *애플리케이션* 탭에 있는 애플리케이션 목록에 나타납니다. NetApp Backup and Recovery는 설정에 따라 애플리케이션을 보호할 수 있으며, *모니터링* 영역에서 진행 상황을 확인할 수 있습니다.
...
+
*** *metadata.annotations.protect.trident.netapp.io/skip-vm-freeze*: (_선택 사항_) 이 어노테이션은 KubeVirt 환경과 같이 스냅샷 전에 파일 시스템이 동결되는 가상 머신에서 정의된 애플리케이션에만 적용됩니다. 이 애플리케이션이 스냅샷 중에 파일 시스템에 쓸 수 있는지 여부를 지정합니다. true로 설정하면 애플리케이션은 전역 설정을 무시하고 스냅샷 중에 파일 시스템에 쓸 수 있습니다. false로 설정하면 애플리케이션은 전역 설정을 무시하고 스냅샷 중에 파일 시스템이 동결됩니다. 지정되었지만 애플리케이션 정의에 가상 머신이 없는 경우 어노테이션은 무시됩니다. 지정하지 않으면 애플리케이션은 https://docs.netapp.com/us-en/trident/trident-protect/trident-protect-requirements.html#protecting-data-with-kubevirt-vms["전역 파일 시스템 고정 설정"]를 따릅니다.
