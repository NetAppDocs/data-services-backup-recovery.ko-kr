...
[role="lead"]
NetApp Backup and Recovery for ONTAP 볼륨을 사용하기 전에 다음 제한 사항을 검토하십시오.
...
* FlexGroup 볼륨은 한 번에 하나씩만 복제 대상으로 선택할 수 있습니다. 각 FlexGroup 볼륨에 대해 백업을 별도로 활성화해야 합니다.
+
FlexVol 볼륨에는 이러한 제한이 없습니다. 시스템의 모든 FlexVol 볼륨을 선택하고 동일한 백업 정책을 할당할 수 있습니다.
...
* FlexVol 볼륨을 FlexGroup 볼륨으로 또는 그 반대로 변환하면 변환된 볼륨은 새로운 볼륨으로 간주됩니다. 따라서 변환 후 변환된 볼륨에 대한 보호 설정을 해야 합니다. 변환 전에 생성된 백업은 다른 볼륨으로 복원할 수 있습니다.
...
* 장기 보존 스냅샷이 활성화된 SnapMirror 타겟 볼륨은 백업하지 마십시오. 이러한 스냅샷은 타겟에 직접 생성되며 객체 스토리지로 이동되지 않습니다. 이 제한 사항은 SnapMirror 정책이 나중에 장기 보존을 비활성화하도록 변경되더라도 적용됩니다.
* 볼륨이 정책에 할당되지 않은 상태에서 백업 정책을 생성하거나 편집하는 경우 보관되는 백업 수는 최대 1018개입니다.  정책에 볼륨을 할당한 후 정책을 편집하여 최대 4000개의 백업을 만들 수 있습니다.
* 데이터 보호(DP) 볼륨을 백업할 때:
+
** SnapMirror 레이블 `app_consistent` 및 `all_source_snapshot`과(와) 관련된 관계는 클라우드에 백업되지 않습니다.
** SnapMirror 타겟 볼륨에 스냅샷의 로컬 복사본을 생성하는 경우(사용된 SnapMirror 레이블과 관계없이) 이러한 스냅샷은 백업으로 클라우드로 이동되지 않습니다. NetApp Backup and Recovery에서 스냅샷을 백업할 수 있도록 소스 DP 볼륨에 원하는 레이블이 포함된 스냅샷 정책을 생성해야 합니다.
...
** 볼륨에 적용된 스냅샷 정책은 일별, 주별, 월별 등 NetApp Backup and Recovery에서 인식하는 정책 중 하나여야 합니다. 기본 정책인 "sm_created"(*모든 스냅샷 미러링*에 사용됨)는 인식되지 않으며, 이 경우 DP 볼륨은 백업 가능한 볼륨 목록에 표시되지 않습니다.
** SVM-DR과 볼륨 백업 및 복구는 백업이 소스 또는 대상에서 수행될 때 완전히 독립적으로 작동합니다. 유일한 제약 사항은 SVM-DR이 SnapMirror 클라우드 관계를 복제하지 않는다는 것입니다. 재해 복구 시나리오에서 스토리지 VM이 보조 위치에서 온라인 상태가 되면 SnapMirror 클라우드 관계를 수동으로 업데이트해야 합니다.


* MetroCluster 지원:
+
** ONTAP 9.12.1 GA 이상 버전을 사용하는 경우, 주 시스템에 연결된 상태에서 백업이 지원됩니다. 전체 백업 구성이 2차 시스템으로 전송되므로 전환 후에도 클라우드 백업이 자동으로 계속됩니다. 2차 시스템에서 백업을 설정할 필요가 없습니다(실제로 설정하지 않도록 제한되어 있습니다).
** ONTAP 9.12.0 및 이전 버전을 사용하는 경우 백업은 ONTAP 보조 시스템에서만 지원됩니다.
** ONTAP 9.18.1 버전부터 FlexGroup 볼륨 백업이 MetroCluster 구성에서 지원됩니다.


* 데이터 보호 볼륨에서는 *지금 백업* 버튼을 사용한 임시 볼륨 백업이 지원되지 않습니다.
* SM-BC 구성은 지원되지 않습니다.
* ONTAP는 단일 볼륨에서 여러 오브젝트 저장소로의 SnapMirror 관계 팬아웃을 지원하지 않으므로, 이 구성은 NetApp Backup and Recovery에서 지원되지 않습니다.
* 현재 Amazon S3, Google Cloud Platform(GCP), Azure 및 StorageGRID에서 객체 저장소의 WORM/규정 준수 모드가 지원됩니다. 이는 DataLock 기능으로 알려져 있으며, 클라우드 공급자 인터페이스가 아닌 NetApp Backup and Recovery 설정을 사용하여 관리해야 합니다.
...
* 로컬 스냅샷에서 폴더를 복원하는 것은 지원되지 않습니다.
* 재해 복구(DR) 구성에서 파일 및 폴더 복원: 페일오버 후, DR용으로 구성된 스토리지 VM의 볼륨은 기존 SnapMirror 관계를 끊어진 상태로 유지합니다. 임시 SnapMirror RST 관계를 생성하는 파일 또는 폴더 복원 시도는 대상 볼륨에 이미 관계가 존재하기 때문에 ONTAP에서 차단됩니다.
* 찾아보기 및 복원 기능은 한 번에 최대 100개의 개별 파일을 복원할 수 있습니다.
* 검색 및 복원 기능을 사용하면 한 번에 하나의 파일만 복원할 수 있습니다.
...
재해 복구(DR) 구성에서 스토리지 VM 클러스터에 대한 인덱싱을 활성화할 때 Vserver DR의 대상으로 구성된 Vserver에서 해당 작업이 허용되지 않는다는 오류 메시지가 나타납니다. 이 오류에도 불구하고 백엔드 프로세스는 `snapdiff-rpc-server` 소스 스토리지 VM의 상태를 "켜짐"으로 설정합니다. 스토리지 VM DR 정책 스케줄이 실행된 후 이 상태가 대상 스토리지 VM으로 전파됩니다.

*해결 방법*: 초기 오류 발생 후 스토리지 VM DR 정책 일정이 실행될 때까지 기다린 다음 인덱싱을 다시 활성화해 보세요.
