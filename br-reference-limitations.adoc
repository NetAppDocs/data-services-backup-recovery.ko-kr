...
** SnapMirror 대상 볼륨에 스냅샷의 로컬 복사본을 생성하는 경우(사용된 SnapMirror 레이블과 관계없이) 이러한 스냅샷은 백업으로 클라우드로 이동되지 않습니다. NetApp Backup and Recovery에서 스냅샷을 백업할 수 있도록 소스 DP 볼륨에 원하는 레이블이 포함된 스냅샷 정책을 생성해야 합니다.
